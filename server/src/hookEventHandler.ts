...
export class HookEventHandler {
  ...
  /** Primary provider (first registered). Used for JSONL/team features. */
  ...
  constructor(
    ...
    const first = [...providers.values()][0];
    if (!first) throw new Error('[Pixel Agents] HookEventHandler requires at least one provider');
    this.provider = first;
  ...
  handleEvent(providerId: string, event: HookEvent): void {
    // Look up the provider that sent this event; fall back to the primary provider.
    const provider = this.providers.get(providerId) ?? this.provider;
    ...
    // Downstream dispatch uses only the normalized AgentEvent.kind.
    ...
    agent.hookDelivered = true;
    if (agent.hooksOnly) agent.lastDataAt = Date.now();
    if (debug)
      console.log(
    ...
    switch (normEvent.kind) {
      ...
      case 'toolStart':
        // Pass the resolved provider so formatToolStatus uses the right formatter.
